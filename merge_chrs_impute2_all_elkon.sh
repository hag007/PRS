...
for a in "${chrs_range[@]}"; do 
    ( ( ls -1v ${target_path}raw/impute2/parts/chr${a}.*.legend | xargs cat > ${target_path_elkon}/raw/impute2/chrs/chr${a}.impute2 ) && echo "done merging chr${a}" ) & pids+=($!); 
done;

while [[ ${#pids[@]}  -gt 0 ]]; do
    for i_pid in "${!pids[@]}"; do
         pid=${pids[i_pid]}
         if  [[ $(kill -0 $pid  2>&1) ]]  ; then
             echo "remove index $counter: ${pids[i_pid]}"
             unset 'pids[i_pid]'             
         fi
    done
    echo "Waiting for ${#pids[@]} chrs to be merged"
    echo "cur pids: ${pids[@]}"
    echo "cur pid indices: ${!pids[@]}"
    sleep 60
done
