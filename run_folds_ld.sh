    ...
    for target in ${targets[@]}; do
        for imp in ${imps[@]}; do
            for cur_rep in `seq ${rep_start} ${rep_end}`; do
                ...
                res3=$(ls -1 /specific/netapp5/gaga/gaga-pd/prs_data/PRSs//${discovery}_${target}/${imp}/rep_${base_rep}_${cur_rep}/prs.cv.${method}_${pheno}__${folds}_test.ctrl.or.summary.*.tsv 2>/dev/null | wc -l );
