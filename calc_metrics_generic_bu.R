...
percentile.or <- function(pheno.prs, resolution=0.01){
...
cox.regression <- function(pheno.prs, phenotype.file.name){
  ...
  prs.coef <- summary(prs.model)$coeff["SCORE_SCALE",]
  prs.beta <- as.numeric(prs.coef[1])
  prs.se <- as.numeric(prs.coef[2])
  prs.p <- as.numeric(prs.coef[4])
  
  return(data.frame(cox.exp.beta=prs.coef[2], cox.se=prs.coef[3], cox.pval=prs.coef[5]))
  
}

percentile.hr <- function(pheno.prs, phenotype.file.name, resolution=0.01){
    df.or=data.frame()
...
format.beta.by.percentile <- function(beta.by.percentile.results, resolution, prefix){
  ...
  h.t<-paste(prefix,percentile.t, sep='_')
...
calc.metrics <- function(target.path, imp.name, res.path, sub, grid.ids, prs.prefix, suffix, rep, analysis.type, resolution=0.01){
    ...
    for(i in grid.ids){
        ...
        cox.by.percentile.summary<-format.beta.by.percentile(cox.by.percentile.results[7:nrow(cox.by.percentile.results),c(2,6,7)], resolution, "hr")
        prs.result<-cbind(prs.result, cox.by.percentile.summary)
          
        print('Save statistics and metrics')
        prs.result<-cbind(data.frame(hp=i), prs.result)
        ...
        all.prs.results=rbind(all.prs.results,prs.result)
        
        print(paste0('=== End analyzing hp: ',i, " ==="))

    }
    # write.table(all.prs.results, paste(res.path,paste0(prs.prefix,suffix,".statistics.tsv"), sep='/'), row.names = FALSE, sep='\t', quote = FALSE)
    print(all.prs.results)
    print(paste("Saved full statistics in", paste(res.path,paste0(prs.prefix,suffix,".statistics.tsv"), sep='/')))
}

# target.path<-"/specific/netapp5/gaga/gaga-pd/prs_data/datasets/dec/cimba_eur_brca1_oncoarray"
# imp.name<-"impX_313"
# res.path<-"/specific/netapp5/gaga/gaga-pd/prs_data/PRSs/bca_313_neg_cimba_eur_brca1_oncoarray/impX_313"
# sub<-""
# grid.ids<-c("-1")
# prs.prefix<-"prs.mono.pt"
#  suffix<-""
# rep<-""
# analysis.type<-"mono"
# cov.path<-"/specific/netapp5/gaga/gaga-pd/prs_data/datasets/dec/cimba_eur_brca1_oncoarray/cov"
# calc.metrics(target.path, imp.name, res.path, cov.path, sub, grid.ids, prs.prefix, suffix, rep, analysis.type)
