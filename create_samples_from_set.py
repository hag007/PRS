...
if __name__=="__main__":
    ...
    df_control=df_all.loc[df_all.loc[:,'label']==1]
    df_case=df_all.loc[df_all.loc[:,'label']==2]

    for n_sample in range(1,9): # while df_case.shape[0]>=base_case+additional_case and df_control.shape[0]>=base_control+additional_control:
        ...
        df_base=pd.concat((df_case.iloc[:base_case], df_control.iloc[:base_control]))
        df_full=pd.concat((df_case.iloc[:base_case+additional_case], df_control.iloc[:base_control+additional_control]))

        base_sample_path = os.path.join(constants.DATASETS_PATH, dataset, f'{pheno_folder}_{n_sample}', f'pheno_{phenotype}_{population}_1_2_sample')
        full_sample_path = os.path.join(constants.DATASETS_PATH, dataset, f'{pheno_folder}_{n_sample}', f'pheno_{phenotype}_{population}_2_2_sample')

        df_base.to_csv(base_sample_path, sep='\t', index=None)
        df_full.to_csv(full_sample_path, sep='\t', index=None)
