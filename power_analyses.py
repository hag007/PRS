    ...
    for n_samples in ar_n_samples:
        for prevalence in ar_prevalences:
            ...
            while a < n_iterations:
                print(f'### current execution: cur_cohort_size={n_samples} cur_prevalence={prevalence} cur_iteration={a} ###')
                df=df.sample(frac=1)
                ...
                child=subprocess.Popen(f'bash calc_prs_fit.sh --discovery {discovery} --target {target} --imp {imp}', shell=True, stdout=subprocess.PIPE, cwd=scripts_path)
