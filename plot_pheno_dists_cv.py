...
if __name__=='__main__':
    ...
    parser.add_argument('-th', '--thresholds', dest='thresholds', default="0.5-11", help='')     # 0.001,0.05,0.1,0.2,0.5
    ...
    for th in thresholds:
        for discovery in gwass:
            for target in targets:
                ...
                df=pd.DataFrame()
                for a in range(71,91):
                    if os.path.exists(pheno_path):
                        print("exists")
                        df_pheno=pd.read_csv(pheno_path, sep='\t', index_col=0)
                        df_pheno.index=df_pheno.index.astype(str)
                        df_statistics=pd.concat((df_statistics, pd.read_csv(os.path.join(constants.PRSS_PATH, f'{discovery}_{target}',imp, f"rep_{a}", f'prs.cv.ls___5_test.statistics.{th}.tsv'), sep='\t'))) # , index_col=0
                        df_or_percentile=pd.concat((df_or_percentile, pd.read_csv(os.path.join(constants.PRSS_PATH, f'{discovery}_{target}',imp,f"rep_{a}",f'prs.cv.ls___5_test.or.percentile.{th}.tsv'), sep='\t')))
                        df_or_p_value=pd.concat((df_or_p_value, pd.read_csv(os.path.join(constants.PRSS_PATH, f'{discovery}_{target}',imp,f"rep_{a}",f'prs.cv.ls___5_test.or.p.value.{th}.tsv'), sep='\t')))
                        df_or_all=pd.concat((df_or_all, pd.read_csv(os.path.join(constants.PRSS_PATH, f'{discovery}_{target}',imp,f"rep_{a}",f'prs.cv.ls___5_test.or.all.{th}.tsv'), sep='\t')))


                    print(os.path.join(constants.PRSS_PATH, f'{discovery}_{target}',imp, "rep_72", f'prs.cv.ls___5_test{"."+th if th!="" else ""}.profile'))
                    df=pd.concat((df, pd.read_csv(os.path.join(constants.PRSS_PATH, f'{discovery}_{target}',imp, f"rep_{a}", f'prs.cv.ls___5_test{"."+th if th!="" else ""}.profile'), delim_whitespace=True))) # , index_col=0
                ...
                pop_means=[]
                pop_stds=[]
                fig, ax = plt.subplots(1,1,figsize=(22,21))
                ax2=ax.twinx()
                ar_vals=[]
                vals_max=-100
                vals_min=100
                ...
                if target.startswith('hapmap') or target.startswith('chrs_full')   or True:
                    ...
                    for i, pop in enumerate(populations_unique): # enumerate(populations_unique):
                        ...
                        print("pop_vals:", pop_vals)
                        print(df_pheno)
                        if not df_pheno is None:
                            df_cur_pheno=  pd.merge(pop_vals, df_pheno, on='FID').dropna() # df_pheno.reindex(pop_vals.iloc[:,0]).loc[:, 'label'].dropna()
                ...
                for i, pop in enumerate(super_populations_unique):
                    ...
                    if not df_pheno is None:
                        ...
                        for cur_pheno_val in unique_pheno:
                            ...
                            ks=scipy.stats.gaussian_kde(cur_pop_by_pheno_val.loc[:,"SCORE"].values)
