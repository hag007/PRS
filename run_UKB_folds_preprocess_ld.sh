    ...
    for target in ${targets[@]}; do
        for imp in ${imps[@]}; do
            for cur_rep in `seq ${rep_start} ${rep_end}`; do
                for fold in `seq ${fold_start} ${fold_end}`; do
                    ...
                    if [[ ${stage} -ne -1 ]]; then
                        if [[ ${complete_missing} = true ]]; then
                            bash calc_prs_cv_preprocess_${method}.sh --discovery ${discovery} --target ${target} --imp ${imp} --cv ${fold}_${folds} --rep ${base_rep}_${cur_rep}  --stage ${stage} --pheno ${pheno} --ld ${ld};
                        fi
                        counter=$((${counter}+1))
                    fi
                done;
                ...
                if [[ ${stage} -ne -1 ]]; then
                    if [[ ${complete_missing} = true ]]; then
                        bash calc_prs_cv_preprocess_${method}.sh --discovery ${discovery} --target ${target} --imp ${imp} --cv ${folds} --rep ${base_rep}_${cur_rep}  --stage ${stage} --pheno ${pheno} --ld ${ld};
                    fi
                    counter=$((${counter}+1))
                fi
            done
