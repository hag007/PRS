...
paste <(gunzip -c $PRS_DATASETS_ELKON/cimba/original/raw/215_brca${brca_type}_oncoarray_imputed_dosages_chr${chr}.txt.gz | awk '{if (substr($1,1,2)=="rs"){split($1,a,":"); print a[1]"\t"$3"\t"$4}}') <(gunzip -c $PRS_DATASETS_ELKON/cimba/original/raw/215_brca${brca_type}_oncoarray_imputed_dosages_chr${chr}.txt.gz | awk -v OFS="\t" '{if (substr($1,1,2)=="rs"){$1=$1; print $0}}' | cut -f 5- ) >> $PRS_DATASETS_ELKON/cimba/original/raw/dat/brca${brca_type}_chr${chr}.dat
