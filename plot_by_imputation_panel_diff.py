...
def plot_or_by_imputation(file_name_format, prs_names, targets, imps, methods=['pt', 'pt2', 'ls'], cols=3, metric="all", suffix=""):
    ...
    for i, method in enumerate(methods):
        ...
        shift=df_ref.reindex(df.loc[:,'prs_name']).loc[:,f'mean_outer_{metric}']
        baseline[(baseline.values<1.0) & (shift.values<1.0)]=np.nan

        df.loc[:,f'mean_outer_{metric}']=baseline.values-shift.values
        # print(suffix, method)
        # print(df)

        # df.loc[:,f'mean_outer_{metric}']=df.loc[:,f'mean_outer_{metric}'].values-df_ref.reindex(df.loc[:,'prs_name']).loc[:,f'mean_outer_{metric}'].values
        ...
        ax.plot([-1,len(imps)-len(targets)+1], [0,0], linestyle='dashed', color='black') # , fmt='-o', linestyle="None", marker='o', markersize=12)
        ...
        if len(df.loc[:,'x_values'].unique()) == len(df.loc[:,"x_values"]):
            ...
            sns.boxplot(x='x_values', y=f"mean_outer_{metric}", data=df, ax=ax, color=sns.color_palette('pastel')[0],  showfliers=False, showmeans=True, meanprops={"markersize":"14", "markerfacecolor": "red"}) # , fmt='-o', linestyle="None", marker='o', markersize=12)
    ...
    ####################
    ax=axs[-1][-1]
    sns.boxplot(x='x_values', y=f"mean_outer_{metric}", data=df_agg, ax=ax, color=sns.color_palette('pastel')[0],  showfliers=False, showmeans=True, meanprops={"markersize":"14", "markerfacecolor": "red"}) # , fmt='-o', linestyle="None", marker='o', markersize=12)
    ...
    i=0
    for imp in imps[:-len(targets)]:
        vals=df_agg.loc[df_agg.loc[:,'imp']==imp,f'mean_outer_{metric}'].dropna().values
        if len(vals)!=0:
            pval=sp.stats.wilcoxon(vals).pvalue
            print(pval)
            ax.text(i-0.25,df_agg.loc[:,'mean_outer_all'].max()+0.05, f'{pval:.2e}')
            i+=1
    plt.setp(ax.get_xticklabels(), rotation=35)
    ax.set_xlabel("imputation panels")
    ax.set_ylabel(d_metrics.get(metric,metric))
...
if __name__=='__main__':
    ...
    imps_pop=["impute2_1kg_ceu2", "impute2_1kg_gbr2", "impute2_1kg_tsi2", "impute2_1kg_ibs2", "impute2_1kg_fin2", "impute2_1kg_eur2", "impute2_ajkg14_t101"]
    discoveries=["PGC2_noAJ"]
    # plot_or_by_imputation(file_name_format.format(suffix, f'{base_rep}_{rep_start}_{base_rep}_{rep_end}'), targets=[target], prs_names=prs_names, imps=imps_super_pop, suffix="super_pop")
    # plot_or_by_imputation(file_name_format.format(suffix, f'{base_rep}_{rep_start}_{base_rep}_{rep_end}'), discoveries=[] targets=[target], prs_names=prs_names, imps=imps_pop, suffix="pop")
    #
    ...
    # plot_or_by_imputation(file_name_format.format(suffix, f'{base_rep}_{rep_start}_{base_rep}_{rep_end}'), prs_names=prs_names, targets=[target], imps=imps_super_pop, suffix="super_pop")
    ...
    plot_or_by_imputation(file_name_format.format(suffix, f'{base_rep}_{rep_start}_{base_rep}_{rep_end}'), prs_names=prs_names, targets=targets, imps=imps_super_pop, suffix=f"all_{suffix}_super_pop")
    # plot_or_by_imputation(file_name_format.format(suffix, f'{base_rep}_{rep_start}_{base_rep}_{rep_end}'), prs_names=prs_names, targets=targets, imps=imps_super_pop, metric="99", suffix=f"all_{suffix}_super_pop")
    ...
    targets=["ukbb_sas", "ukbb_afr"] # , "ukbb_afr"] # ,
    imps_super_pop=["original", "impute2_1kg_eur", "impute2_1kg_sas", "impute2_1kg_afr"] #
    plot_or_by_imputation(file_name_format.format(suffix, f'{base_rep}_{rep_start}_{base_rep}_{rep_end}'), prs_names=prs_names, targets=targets, imps=imps_super_pop, suffix=f"all_{suffix}_super_pop")
    # plot_or_by_imputation(file_name_format.format(suffix, f'{base_rep}_{rep_start}_{base_rep}_{rep_end}'), prs_names=prs_names, targets=targets, imps=imps_super_pop, metric="99", suffix=f"all_{suffix}_super_pop")
