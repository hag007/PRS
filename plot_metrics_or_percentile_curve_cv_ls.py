...
def plot_percentile_curve_multi(fname, fname_test, prs_name, imp, rep_start, rep_end, hyperparameters, folds=5, cols=3):
    ...
    start=int(rep_start.split("_")[1])
    end=int(rep_end.split("_")[1])
    for rep in range(start,end+1):
        for hyperparameter in hyperparameters:
            for fold in range(1,folds+1):
                # print(rep, hyperparameter,fold)
                cur_df=pd.read_csv(os.path.join(constants.PRSS_PATH, prs_name, imp, f'rep_{rep_start.split("_")[0]}_{rep}', fname.format(fold, folds, hyperparameter)), sep='\t')
                cur_df.loc[:,'hyperparameter']=hyperparameter
                df=pd.concat((df,cur_df))

            cur_df_test=pd.read_csv(os.path.join(constants.PRSS_PATH, prs_name, imp, f'rep_{rep_start.split("_")[0]}_{rep}', fname_test.format(folds,hyperparameter)), sep='\t')
    ...
    for set_type in ["validation", "test"]:
        ...
        for i, hp in enumerate(hyperparameters):
            ...
            if set_type=="validation":
                ...
                sd=df_test[df_test.loc[:,'hyperparameter']==hp].groupby(groupby_colname).sem()
