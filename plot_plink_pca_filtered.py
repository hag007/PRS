...
if __name__=='__main__':    
    ...
    parser.add_argument('-d', '--discovery', dest='gwass', help='', default="LH_PGC-SCZ")
    parser.add_argument('-t', '--target', dest='targets',  default="chrs_full_aj_dataset_2018", help='')    
    parser.add_argument('-th', '--thresholds', dest='thresholds', default="0.1", help='')    
    ...
    parser.add_argument('-ref_p', '--reference_pop', dest='reference_pop', default="ACB,ASW,ESN,GWD,LWK,MSL,YRI", help='')
    ...
    reference_pop=args.reference_pop.split(',')
...
## 
                ...
                df.iloc[:,1:]=df.iloc[:,1:].apply(lambda a: (a-a.min())/(a.max()-a.min()))
                labels=labels.loc[df.index]
                mask=(labels.isin(reference_pop))
                df_ref=df[mask].iloc[:,[0,1,2]]
                df=df[df.apply(lambda cur_row: (df_ref.apply(lambda a: sum([(cur_row.iloc[i]- a.iloc[i])**2 for i in range(1,3)])**0.5, axis=1)<0.1).sum()>=10 ,axis=1)]
                df.index.to_frame().to_csv(os.path.join(constants.OUTPUT_PATH, f'pca_filtered_{target}_{pop_type}_{imp}.tsv'), sep='\t', index=None)
