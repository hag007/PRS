        ...
        for hps in "${!a_vals[@]}"; do
            ...
            for chr in {1..22}; do 
                python PRScsx.py \
                     ...
                     --meta=$meta_flag --seed=$seed --chrome=$chr \
                     $phi_param $chrom_param $n_iter_param $n_burnin_param \
                     --thin=$mcmc_thinning_factor --out_dir=${output_dir} --out_name=out
           done

           cd $PRS_CODEBASE
        
            # concatenate all chromosomes' weights into one file 
            for pop in ${gwas_pops[@]}; do 
                cat ${output_dir}/out_${pop}_pst_eff_a1_b0.5_phiauto_chr*.txt \
                    > ${output_dir}/out_all
            done
 ...
 for hps in "${!a_vals[@]}"; do
         ...
         for chr in {1..22}; do 
             if [[ ! -f $output_dir/out_EUR_pst_eff_a${param_a}_b${param_b}_phiauto_chr$chr.txt ]]; then
                ...
                echo "The weights of chr${chr} already exist. Skipping..."
            fi
        done
        ...
        # concatenate all chromosomes' weights into one file 
        for pop in ${gwas_pops[@]}; do 
            cat $output_dir/out_${pop}_pst_eff_a${param_a}_b${param_b}_phiauto_chr*.txt \
                > $output_dir/out_all
        done
