...
for discovery in ${discoveries[@]}; do
    ...
    echo "gwa_to_pheno: ${gwas_to_pheno}"
    echo "pheno: $pheno"
        ...
        for imp in ${imps[@]}; do
            for cur_rep in {1..6}; do
                for fold in {1..5}; do
                    ...
                    res3=$(ls -1 /specific/netapp5/gaga/gaga-pd/prs_data/PRSs//${discovery}_${target}/${imp}/rep_${base_rep}_${cur_rep}/prs.cv.${method}_${pheno}__${fold}_${folds}_validation.or.percentile.1-*.tsv 2>/dev/null | wc -l );
                    stage=-1
                    ...
                    if [[ ${stage} -ne -1 ]]; then
                        if [[ ${complete_missing} = true ]]; then
                            bash calc_prs_cv_${method}.sh --discovery ${discovery} --target ${target} --imp ${imp} --cv ${fold}_${folds} --rep ${base_rep}_${cur_rep} --ld ${ld} --pheno ${pheno} --stage ${stage}; 
                        fi
                        counter=$((${counter}+1))
                    fi
                done;
                ...
                res3=$(ls -1 /specific/netapp5/gaga/gaga-pd/prs_data/PRSs//${discovery}_${target}/${imp}/rep_${base_rep}_${cur_rep}/prs.cv.${method}_${pheno}__${folds}_test.or.percentile.1-*.tsv 2>/dev/null | wc -l );
                ...
                if [[ ${stage} -ne -1 ]]; then
                    if [[ ${complete_missing} = true ]]; then
                        bash calc_prs_cv_${method}.sh --discovery ${discovery} --target ${target} --imp ${imp} --cv 5 --rep ${base_rep}_${cur_rep} --ld ${ld} --pheno ${pheno} --stage ${stage} ;
                    fi
                    counter=$((${counter}+1))
                fi
            done
