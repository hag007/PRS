...
calc.metrics <- function(src.path, imp.name, res.path, sub, grid.ids, prs.prefix, suffix, rep){
    ...
    # print(paste0("dim before filtering: ", dim(phenotype)[1]))
    # brca.list <- read.table(paste(target.path, "brca_carriers", sep="/"), header=F, stringsAsFactors=F)
    # phenotype <- phenotype[!(phenotype[,'FID'] %in% unlist(brca.list)) ,]
    # aj.outliers.list <- read.table(paste(target.path, "aj_outliers", sep="/"), header=F, stringsAsFactors=F )
    # phenotype <- phenotype[!(phenotype[,'FID'] %in% unlist(aj.outliers.list)), ]
    # print(paste0("dim after filtering: ", dim(phenotype)[1]))
    ...
    pcs <- read.table(paste(src.path, imp.name, paste(paste0(eigenvec.prefix, suffix,".eigenvec")), sep="/"), header=F)
    pcs <- pcs[,1:8]
    ...
    for(i in grid.ids){
        print('Read PRS')
        print(paste("prs.prefix",prs.prefix))
        ...
        write.table(df.or.all, paste(res.path,paste0(prs.prefix,suffix,".ctrl.or.all.",i,".tsv"), sep='/'), row.names = FALSE, sep='\t', quote = FALSE)
        ...
        write.table(or.fit$measure, paste(res.path,paste0(prs.prefix,suffix,".ctrl.or.percentile.",i,".tsv"), sep='/'), col.names= NA, row.names = TRUE, sep='\t', quote = FALSE)
        write.table(or.fit$p.value, paste(res.path,paste0(prs.prefix,suffix,".ctrl.or.p.value.",i,".tsv"), sep='/'), row.names = FALSE, sep='\t', quote = FALSE)
        ...
        res.or.analysis <- data.frame(threshold=i, 
            or_all=df.or.all[nrow(df.or.all),1], or_all_se=df.or.all[nrow(df.or.all),4], or_all_ci_min=df.or.all[nrow(df.or.all),2], or_all_ci_max=df.or.all[nrow(df.or.all),3], or_all_power=res.wp,
        ...
        write.table(res.or.analysis, paste(res.path,paste0(prs.prefix,suffix,".ctrl.or.summary.",i,".tsv"), sep='/'), row.names = FALSE, sep='\t', quote = FALSE)
        
    
        print('print statistics and metrics (w/o OR)')
        write.table(prs.result, paste(res.path,paste0(prs.prefix,suffix,".ctrl.statistics.",i,".tsv"), sep='/'), row.names = FALSE, sep='\t', quote = FALSE)
